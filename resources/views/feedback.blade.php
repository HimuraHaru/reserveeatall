...
@section('content')
                                                ...
                                                <div class="half">
                                                    <p class="per-head">
                                                        @if(\App\Like::where('userID', \App\Http\Helpers::userID())->where('feedbackID', $feedback->feedbackID)->count() == 0)
                                                            <a style="text-decoration: none; color:inherit;"  href="{{ route('like-feedback', [$feedback->feedbackID, $restaurant->restaurantID, 'feedback'] ) }}">
                                                ...
                                                {{--<div class="half">--}}
                                                    {{--<p class="price">--}}
                                                        {{--@if(\App\Dislike::where('userID', \App\Http\Helpers::userID())->where('feedbackID', $feedback->feedbackID)->count() == 0)--}}
                                                            {{--<a style="text-decoration: none; color:inherit;" href="{{ route('dislike-feedback', [$feedback->feedbackID, $restaurant->restaurantID, 'feedback'] ) }}">--}}
    ...
    <h2 class="section-title">Thanks for the Reviews & Feedbacks</h2>
    </div>
